...
export const ActiveTracker: React.FC<ActiveTrackerProps & { isCloudEnabled?: boolean }> = ({ 
  ...
  // Timer untuk reset status konfirmasi jika tidak diklik lagi dalam 3 detik
  useEffect(() => {
  ...
  const handleCancelClick = () => {
    if (isConfirmingCancel) {
      onCancel(); // Eksekusi batal (kembali ke input tanpa simpan)
    } else {
      setIsConfirmingCancel(true); // Masuk mode konfirmasi
    }
  };
  ...
  return (
    <div className="flex flex-col items-center space-y-8 max-w-2xl mx-auto w-full">
      ...
      <div className="w-full flex flex-col space-y-4">
        ...
        {/* TOMBOL BATAL DENGAN KONFIRMASI INTERNAL */}
        <button 
          type="button"
          onClick={handleCancelClick}
          className={`mx-auto flex items-center space-x-2 text-[11px] font-black uppercase tracking-[0.2em] transition-all py-3 px-6 rounded-2xl active:scale-95 border-none outline-none ${
            isConfirmingCancel 
              ? 'bg-red-600 text-white shadow-lg shadow-red-500/30 scale-105 animate-bounce' 
              : 'text-slate-400 hover:text-red-500 bg-slate-100/50 sm:bg-transparent'
          }`}
        >
          <svg className={`w-3.5 h-3.5 ${isConfirmingCancel ? 'animate-pulse' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            {isConfirmingCancel ? (
          ...
          <span>{isConfirmingCancel ? 'YAKIN BATAL? (Klik Lagi)' : 'Batal & Discard Sesi Ini'}</span>
        </button>
      </div>
    </div>
  );
