...
export const ActiveTracker: React.FC<ActiveTrackerProps & { isCloudEnabled?: boolean }> = ({ 
  ...
  // Otomatis reset status konfirmasi jika tidak diklik lagi dalam 3 detik
  useEffect(() => {
  ...
  const handleCancelClick = (e: React.MouseEvent) => {
    e.preventDefault();
    if (isConfirmingCancel) {
      // Eksekusi Batal (Discard data sepenuhnya)
      onCancel();
    } else {
  ...
  return (
    <div className="flex flex-col items-center space-y-8 max-w-2xl mx-auto w-full animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="w-full flex flex-col space-y-4">
        ...
        {/* TOMBOL BATAL - TERSEDIA UNTUK SEMUA USER */}
        <button 
          type="button"
        ...
        >
          ...
          <span>{isConfirmingCancel ? 'KLIK LAGI UNTUK DISCARD' : 'Batal & Buang Sesi'}</span>
        </button>
      </div>
    </div>
  );
